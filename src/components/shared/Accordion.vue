...
<section class="faq mt-8" ref="rootEl">
    <transition name="accordion-fade-slide" mode="out-in">
        <div v-if="items.length" class="faq-wrapper">
            ...
            <div v-if="showAccordion" class="accordion">
                <draggable :list="dragArray" v-bind="dragOptions" @change="updateListOrder" >
                    <div
                    class="accordion__item"
                    v-for="(item, i) in categoryItems"
                    :key="`accordion-item-${i}`"
                    >
                    <div class="accordion__title-text p-3" :class="generateQuestionClasses(i)">
                        <component 
                        :is="item.editMode ? 'input' : 'p'" 
                        @change="updateItem(item, 'title', $event)"
                        type="text"
                        :class="[item.editMode ? 'px-6 py-2' : '']"
                        ...
                        <div class="accordion-controls flex items-center">
                            ...
                            <plus-icon @click.prevent="makeActive(i)" :class="generateButtonClasses(i)" />
                        </div>
                    </div>
                        <collapse-transition>
                            <div class="accordion__body px-3 pb-3" v-if="i === activeQuestionIndex">
                                <slot v-bind:item="item">
                                    <component 
                                    :is="item.editMode ? 'textarea' : 'p'" 
                                    @change="updateItem(item, 'body', $event)"
                                    :value="item[answerProperty]"
                                    v-text="item[answerProperty]"
                                    :class="[item.editMode ? 'w-full p-6' : '']"
                                    >
                                    </component>
    ...
    <a class="mt-4 flex" @click.prevent="addItem" href="#"><plus-circle-icon class="mr-2" /> Add Item</a>
</section>
</template>

<script>
import Vue from 'vue'
import { EditIcon, XIcon, TrashIcon, PlusCircleIcon, PlusIcon } from 'vue-feather-icons'
...
export default {
    ...
    methods: {
      ...
      addItem() {
          this.items = [...this.items, { title: '', body: '' }]
      },
      updateItem(item, prop, val) {
        // item[prop] = val.target.value
        const index = this.items.findIndex(el => el.title === item.title)
        this.updateStoreComponent({ path: `content.accordion.items.${index}.${prop}`, prop: val.target.value})
      },
      removeItem(item) {
        const index = this.items.findIndex(el => el.title === item.title)
