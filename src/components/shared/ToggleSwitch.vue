<template>
    <div class="attribute-editor module module-settings mt-0 flex items-center">
        ...
        <label :for="(component.id + 'boxed-layout')" class="pr-4 label-width flex-shrink-0">{{label}}:</label>

        <label class="ui-switch" :class="{ 'checked': value }">
            <input @change="handleChange" :id="(component.id + 'boxed-layout')" type="checkbox" class="ui-switch-input" v-model="value">
...
export default {
    ...
    methods: {
        handleChange() {
            if (!this.path && !this.attribute) {
                this.$emit('toggle', this.value)
            } else {
                this.updateStoreComponent({ path: this.path, prop: this.value })
            }
        }
    },
}
